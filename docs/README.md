...
# 方式一：编辑 bootstrap/src/main/resources/llm-config.yml 设置 API Key
# 方式二：使用环境变量
...
```bash
# 编辑 bootstrap/src/main/resources/llm-config.yml 设置各供应商的 API Key
# 或使用环境变量：
export OPENAI_API_KEY=sk-...
...
#### `llm-config.yml` — LLM 配置（推荐编辑此文件）

所有 LLM 相关参数（API Key、模型、Base URL 等）集中在 `bootstrap/src/main/resources/llm-config.yml` 中。服务启动前直接编辑即可，无需设置环境变量。
...
# ===========================================================
# Edit the values below to configure your LLM providers.
...
scaffold4j:
  ai:
    ...
    providers:
      openai:
        # Your OpenAI API key (set via OPENAI_API_KEY env var)
        ...
        chat:
          ...
          max-tokens: 4096
        embedding:
          # Embedding model for RAG and similarity search
...
**配置优先级**：环境变量 > `llm-config.yml` > 代码默认值

API Key 字段使用 `${ENV_VAR:}` 引用（安全考虑，不硬编码），其他参数（base-url、model 等）直接写值，方便编辑。

#### `application.yml` — 主配置

通过 `spring.config.import` 导入 LLM 配置，其他基础设施配置（数据库、缓存、Nacos 等）仍在 `application.yml` 中：
...
spring:
  application:
    name: my-ai-app
  config:
    import: classpath:llm-config.yml    # 导入 LLM 配置
  datasource:
    url: ${DB_URL:jdbc:postgresql://localhost:5432/my-ai-app}
  cache:
    type: redis
  cloud:
    nacos:
...
cd docker
# 方式一：编辑 bootstrap/src/main/resources/llm-config.yml 设置 API Key 后重新打包
# 方式二：使用环境变量
export OPENAI_API_KEY=sk-...
docker-compose up -d
```
...
**方式一：编辑 `llm-config.yml`**（推荐）

修改 `llm-config.yml` 中的 `default-provider` 和对应供应商的配置：

```yaml
scaffold4j:
  ai:
    default-provider: anthropic    # 切换默认供应商
    providers:
      anthropic:
...
### Q: `llm-config.yml` 和 `application.yml` 是什么关系？

`scaffold4j` 生成两个配置文件来管理不同类型的参数：

| 文件 | 内容 | 编辑频率 |
|------|------|---------|
| `llm-config.yml` | LLM 参数（API Key、模型、Base URL、温度等） | 频繁（切换模型/供应商时） |
| `application.yml` | 基础设施配置（数据库、缓存、Nacos、端口等） | 较少 |

`application.yml` 通过 `spring.config.import: classpath:llm-config.yml` 导入 LLM 配置。Spring Boot 的属性解析优先级为：环境变量 > `application.yml` > `llm-config.yml`。

这种分离的好处：
- LLM 配置独立管理，切换模型/供应商时只需编辑一个文件
- API Key 使用环境变量引用（`${OPENAI_API_KEY:}`），安全不硬编码
- 其他参数（base-url、model 等）直接写值，方便本地开发
