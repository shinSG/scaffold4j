...
public class ConfigGenerator {
        ...
        spring.append("""
                spring:
                  application:
                    name: %s
                  config:
                    import: classpath:llm-config.yml
                  profiles:
    ...
    /**
     * Generate standalone llm-config.yml with all LLM provider configurations.
     * This file is imported into application.yml via spring.config.import.
     */
    public String generateLlmConfig() {
        StringBuilder sb = new StringBuilder();
        ...
        sb.append("""
                ...
                # These properties are imported into application.yml at startup
                # via spring.config.import: classpath:llm-config.yml
                #
                # API keys use environment variable references for security.
                # Set OPENAI_API_KEY, ANTHROPIC_API_KEY, etc. in your
                # environment, or replace the ${...} references with direct
        ...
        // --- scaffold4j unified provider properties ---
        sb.append("""
                # --- scaffold4j unified provider properties ---
                # Used by both Spring AI and LangChain4j adapter classes
                scaffold4j:
                  ai:
        ...
        for (LLMProvider provider : config.llmProviders()) {
            sb.append(generateLlmConfigProviderSection(provider));
        }
        ...
        if (config.usesSpringAI()) {
            sb.append("""
                    # --- Spring AI provider properties ---
                    # Standard Spring AI auto-configuration properties
                    spring:
                      ai:
    ...
    private String generateLlmConfigProviderSection(LLMProvider provider) {
        String envPrefix = envPrefix(provider);
        StringBuilder sb = new StringBuilder();
        sb.append("      ").append(provider.id()).append(":\n");
        sb.append("        # ").append(provider.displayName()).append(" API key\n");
        sb.append("        api-key: ${")
                .append(provider.envVar() != null ? provider.envVar() : envPrefix + "_API_KEY")
                .append(":}\n");
        sb.append("        # Base URL for the ").append(provider.displayName()).append(" API\n");
        sb.append("        base-url: ").append(defaultBaseUrl(provider)).append("\n");

        if (provider == LLMProvider.AZURE_OPENAI) {
            sb.append("        # Azure OpenAI API version\n");
            sb.append("        api-version: 2024-10-21\n");
        }

        sb.append("        chat:\n");
        sb.append("          # Chat completion model name\n");
        sb.append("          model: ").append(defaultModel(provider)).append("\n");
        sb.append("          # Sampling temperature (0.0 - 2.0)\n");
        sb.append("          temperature: 0.7\n");
        sb.append("          # Maximum tokens per response\n");
        sb.append("          max-tokens: 4096\n");

        if (provider == LLMProvider.OPENAI) {
            sb.append("        embedding:\n");
            sb.append("          # Embedding model for RAG and similarity search\n");
